...
const SEVERITY_STYLES: Record<Severity, { color: string; bg: string; label: string }> = {
  low: { color: "text-mint", bg: "bg-mint", label: "Low Impact" },
  medium: { color: "text-amber", bg: "bg-amber", label: "Medium Impact" },
  high: { color: "text-coral", bg: "bg-coral", label: "High Impact" },
  critical: { color: "text-coral", bg: "bg-coral", label: "Critical Impact" },
};

const NEED_STYLES: Record<NeedVerdict, { color: string; bg: string; icon: React.ReactNode; label: string }> = {
  need: { color: "text-mint", bg: "bg-mint/10", icon: <CheckCircle2 size={18} />, label: "Need" },
  want: { color: "text-coral", bg: "bg-coral/10", icon: <Heart size={18} />, label: "Want" },
  mixed: { color: "text-amber", bg: "bg-amber/10", icon: <HelpCircle size={18} />, label: "Mixed" },
};
    ...
    <div className="grid gap-5">
      ...
      <div className="panel p-5">
        <div className="mb-5">
          ...
          <h3 className="mt-1 text-xl font-black">Should I Buy This?</h3>
          <p className="mt-1 text-sm text-muted">Enter a product and price — AI will analyze the impact on your finances.</p>
        </div>

        <div className="grid gap-4 sm:grid-cols-[1fr_180px_auto]">
          <div>
            <label className="mb-1.5 block text-xs font-bold text-muted">Product Name</label>
          ...
          <div>
            <label className="mb-1.5 block text-xs font-bold text-muted">Price</label>
            <div className="relative">
      ...
      {loading && (
        <div className="panel grid min-h-[200px] place-items-center p-8">
          <div className="text-center">
            ...
            <p className="mt-3 text-sm font-bold text-muted">Analyzing purchase impact...</p>
            <p className="mt-1 text-xs text-muted">Checking budgets, goals, and spending patterns</p>
          </div>
        </div>
      )}
          ...
          {analysis.recommendation.alternatives.length > 0 && (
            <div className="panel p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="font-black text-ink">Alternatives to Consider</h4>
              </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-2 text-[11px] font-bold text-muted">
                  Analysis by {analysis.modelName} &middot; Based on your real financial data
                </p>
              </div>
            </div>
          </div>
      ...
      {!analysis && !loading && (
        <div className="panel grid min-h-[260px] place-items-center p-8 text-center">
          <div>
            ...
            <p className="mt-4 text-lg font-black text-ink">Enter a product to analyze</p>
            <p className="mt-2 max-w-md text-sm leading-6 text-muted">
              AI will evaluate whether you should buy it based on your income, budgets, saving goals, and spending patterns.
            </p>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`rounded-full px-3 py-0.5 text-xs font-extrabold ${v.badgeBg}`}>
                {analysis.recommendation.verdict.replace("_", " ").toUpperCase()}
              </span>
            </div>
  ...
  return (
    <div className="panel p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h4 className="font-black text-ink">Need vs Want</h4>
        <span className={`ml-auto rounded-full px-2.5 py-0.5 text-[11px] font-bold ${style.bg} ${style.color}`}>
          {style.label} &middot; {Math.round(data.confidence * 100)}%
        </span>
      </div>
  ...
  return (
    <div className="panel p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h4 className="font-black text-ink">Financial Impact</h4>
      ...
      <div className="mb-3 flex gap-4">
        <div>
          <p className="text-xs font-bold text-muted">Balance After</p>
          <p className={`text-lg font-black ${data.balanceAfter >= 0 ? "text-ink" : "text-coral"}`}>{fmt(data.balanceAfter)}</p>
        </div>
        <div>
          <p className="text-xs font-bold text-muted">Income Ratio</p>
  ...
  return (
    <div className="panel p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h4 className="font-black text-ink">Budget Effect</h4>
      </div>
  ...
  return (
    <div className="panel p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h4 className="font-black text-ink">Goal Impact</h4>
      </div>
          ...
          {data.affectedGoals.map((g, i) => (
            <div key={i} className="flex items-center justify-between rounded-lg bg-amber/5 border border-amber/15 px-3 py-2.5">
              <div>
                ...
                <p className="text-xs font-bold text-muted">New monthly: {fmt(g.newMonthlyRequired)}/month</p>
              </div>
              <div className="text-right">
                <p className="text-lg font-black text-amber">+{g.delayMonths}</p>
                <p className="text-[11px] font-bold text-muted">month{g.delayMonths !== 1 ? "s" : ""} delay</p>
              </div>
            </div>
          ))}
        </div>
